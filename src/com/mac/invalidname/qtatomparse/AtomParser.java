...
public class AtomParser extends Object {
	 ...
	 */
	public static ParsedAtom[] parseAtoms(File f) throws IOException {
	...
	protected static ParsedAtom[] parseAtoms(RandomAccessFile raf, long firstOff, long stopAt) throws IOException {
		...
		while (off < stopAt) {
			...
			// already over
			if (raf.getFilePointer() == stopAt) break;
			...
			if ((atomSize < 0) || ((off + atomSize) > raf.length())) throw new IOException("atom has invalid size: " + atomSize);
			...
			if (ATOM_CONTAINER_TYPES.contains(atomType)) {
				...
				ParsedAtom[] children = parseAtoms(raf, raf.getFilePointer(), off + atomSize);
				parsedAtom = new ParsedContainerAtom(atomSize, atomType, children);
			} else {
				parsedAtom = AtomFactory.getInstance().createAtomFor(atomSize, atomType, raf);
			}
			...
			if (atomType.equals("udta")) off += 4;
			
		} // while not at stopAt
		
		// convert the array list into an array
		ParsedAtom[] atomArray = new ParsedAtom[parsedAtomList.size()];
	 ...
	 */
	protected static final void printAtomTree(ParsedAtom[] atomTree, String indent) {
		for (int i = 0; i < atomTree.length; i++) {
			System.out.print(indent);
			ParsedAtom atom = atomTree[i];
			System.out.println(atom.toString());
			if (atom instanceof ParsedContainerAtom) {
				ParsedAtom[] children = ((ParsedContainerAtom) atom).getChildren();
